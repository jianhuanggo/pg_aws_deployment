export REPO_NAME=pg_finance_trade_test1
...
export API_GATEWAY_ROOT_RES_ID=$(aws apigateway --profile ${AWS_CLIENT} get-resources --rest-api-id ${API_GATEWAY_RES_ID} | jq -r -c '.items[] | select(.path == "/") | .id')
...
aws lambda --profile ${AWS_CLIENT} create-function --function-name lambda-${REPO_NAME} --package-type Image --role arn:aws:iam::${AWS_ACCOUNT_NUMBER}:role/lambda-ex --code ImageUri=${AWS_ACCOUNT_NUMBER}.dkr.ecr.${AWS_REGION}.amazonaws.com/${REPO_NAME}:latest
...
aws apigateway --profile ${AWS_CLIENT} put-integration --rest-api-id ${API_GATEWAY_API_ID} --resource-id ${API_GATEWAY_RES_ID} --http-method GET --type AWS --integration-http-method POST --uri "arn:aws:apigateway:${AWS_REGION}:lambda:path/2015-03-31/functions/arn:aws:lambda:${AWS_REGION}:${AWS_ACCOUNT_NUMBER}:function:lambda-${REPO_NAME}/invocations" --request-templates '{ "application/json": "{\"statusCode\": 200}" }'
...
https://cw8ttqau95.execute-api.us-east-1.amazonaws.com/prod/pg_finance_trade_test1
