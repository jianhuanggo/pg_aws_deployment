...
def get_api_gateway(api_name: str,
                    aws_region: str
                    ) -> Union[str, None]:
    """ws an API Gateway method exists.
    ...
    return [api.get("id") for api in apis.get("items") if api.get("name") == api_name]

    # Check if API with the specified name exists
    # for api in apis.get("items"):
    #     if api['name'] == api_name:
    #         _common_.info_logger(f"API '{api_name}' exists with ID: {api['id']}")
    #         return api['id']
    # return None
